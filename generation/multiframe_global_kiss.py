...
def main(trucksc, val_list, indice, truckscenesyaml, args, config):
    ...
    # --- Initialize lists for final data (potentially refined by ICP) ---
    # If ICP is disabled or fails, the 'unrefined' lists will be used later.
    # If ICP runs successfully, it should populate these 'final' lists with
    # points transformed to the reference ego frame using *refined* poses.
    final_pc_ref_ego_list = []
    final_sem_pc_ref_ego_list = []

    # The variables `refined_lidar_pc_list` and `refined_lidar_pc_with_semantic_list`
    # from the original snippet are replaced by the logic above and the expectation
    # that the ICP block will populate `final_pc_ref_ego_list` etc. if enabled.
    # We don't need to initialize `refined_...` by copying `original_...` here,
    # as the distinction is handled by using 'unrefined' vs 'final' lists.
    ...
    if args.icp_refinement and len(dict_list) > 1:
        ...
        try:
            in_memory_dataset = InMemoryDataset(
                lidar_scans=unrefined_pc_global_list,
        ...
        if args.icp_refinement and pipeline is not None:  # Check again in case init failed
            ...
            try:
                ...
                if not isinstance(estimated_poses_kiss, np.ndarray) or \
                        estimated_poses_kiss.shape != (len(unrefined_pc_global_list), 4, 4):
                    print(f"Error: Unexpected pose results shape {estimated_poses_kiss.shape}. "
                          f"Expected ({len(unrefined_pc_global_list)}, 4, 4). Skipping refinement application.")
        ...
        if args.icp_refinement and estimated_poses_kiss is not None:
            ...
            for idx, points_ego in enumerate(unrefined_pc_global_list):
