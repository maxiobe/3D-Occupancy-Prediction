...
def main(trucksc, val_list, indice, truckscenesyaml, args, config):
    ...
    my_sample = trucksc.get('sample', first_sample_token) # retrieve the first sample as dictionary. Dictionary includes data from multiple sensors
    lidar_data = trucksc.get('sample_data', my_sample['data'][sensor]) # retrieves LiDAR sample data using the specific sensor 'LIDAR_TOP', my_sample['data'][sensor] retrieves token associated with sensor
    # Lidar_data contains sample_token, channel, ego_pose_token, calibrated_sensor_token, filename ...

    lidar_ego_pose0 = trucksc.get('ego_pose', lidar_data['ego_pose_token']) # use the ego pose token from lidar_data to get pose information of ego vehicle when LiDAR data was captured
    # lidar_ego_pose0 contains translation, rotation, timestamp

    lidar_calibrated_sensor0 = trucksc.get('calibrated_sensor', lidar_data['calibrated_sensor_token']) # use calibrated sensor token to load sensor calibration data
    ...
    while True:
        ...
        object_tokens = [truckscenes.get('sample_annotation', box_token)['instance_token'] for box_token in boxes_token] # Uses sample_annotation data to get instance_token fore each bb
        ...
        print(f"Key frame: {lidar_data['is_key_frame']}")
        print(f"Shape of gt_boxes {gt_bbox_3d.shape}")
        ...
        dict = {"object_tokens": object_tokens, # list of tokens that identify objects detected in current frame
                ...
                "lidar_pc": lidar_pc_np, # contains transformed point cloud data (x, y, z coordinates) after processing in common coordinate system (first LiDAR frame)
        ...
        if lidar_data['is_key_frame']: # check if keyframe
            ...
            if args.icp_refinement:
                ...
                current_np_semantic = lidar_pc_with_semantic.points[:, :3]  # Convert current scan to Nx3 numpy
                current_np_labels = lidar_pc_with_semantic.points[:, 3]
                if prev_pcd_np_semantic is not None and current_np_semantic.shape[0] > 50:
                    ...
                    lidar_pc_with_semantic.points = o3d.utility.Vector3dVector(current_np_semantic_combined)
    ...
    print(f"Lidar pc shape {lidar_pc.shape}")
    # print(f"Lidar pc data {lidar_pc}")
    ...
    print(f"Lidar pc semantic shape {lidar_pc_with_semantic.shape}")
    # print(f"Lidar pc semantic data {lidar_pc_with_semantic}")
    ...
    while int(i) < 10000:  # Assuming the sequence does not have more than 10000 frames to avoid infinite loops
        ...
        lidar_path, boxes, _ = truckscenes.get_sample_data(dict['lidar_token']) # retrieve LiDAR path and bounding boxes
        ...
        print(f"Scene points before meshing shape is {scene_points.shape}")
        ...
        print(f"Scene points after meshing shape is {scene_points.shape}")
        ...
        print(f"Dense voxel shape is {dense_voxels_with_semantic.shape}")
        print(f"Occupancy shape is {occ_size}: {occ_size[0]*occ_size[1]*occ_size[2]}")
