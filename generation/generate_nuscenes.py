...
import yaml
import torch
import chamfer # Keep if Chamfer distance is used
import mmcv # Keep for points_in_boxes_cpu
import numpy as np
from nuscenes.nuscenes import NuScenes
from nuscenes.utils import splits
from tqdm import tqdm
# Use NuScenes data classes
from nuscenes.utils.data_classes import Box, LidarPointCloud # +++ Use NuScenes versions +++
# Use NuScenes geometry utils if needed, or keep existing if compatible
from nuscenes.utils.geometry_utils import view_points, transform_matrix # +++ Use NuScenes version +++
from typing import Any, Dict, List, Optional, Union
from pyquaternion import Quaternion
from scipy.interpolate import interp1d
from scipy.spatial.transform import Rotation, Slerp
# from mmdet3d.core.bbox import box_np_ops # Keep if used later (not directly in provided snippet)
from mmcv.ops.points_in_boxes import (points_in_boxes_all, points_in_boxes_cpu, # Keep
                                      points_in_boxes_part)
# from scipy.spatial.transform import Rotation # Keep
# import os.path as osp # Keep
from functools import reduce # Keep
import bisect

import open3d as o3d # Keep
from copy import deepcopy # Keep
import matplotlib.pyplot as plt # Keep


def get_closest_record_optimized(nusc: NuScenes, table_name: str, timestamp: int) -> Dict[str, Any]:
    """
    Finds the record in the specified NuScenes table (e.g., 'ego_pose')
    that has the timestamp closest to the given timestamp using binary search.

    Assumes the table's records are sorted by timestamp.

    Args:
        nusc: The NuScenes instance.
        table_name: The name of the table to search (e.g., 'ego_pose', 'sample').
                    Must be a table accessible via `getattr(nusc, table_name)`
                    and sorted by 'timestamp'.
        timestamp: The target timestamp (in microseconds).

    Returns:
        The record dictionary with the closest timestamp.

    Raises:
        ValueError: If the table is empty, does not exist, or lacks 'timestamp'.
        AssertionError: If the table is not sorted by timestamp (optional check).
    """
    if not hasattr(nusc, table_name):
         raise ValueError(f"NuScenes object does not have table '{table_name}'.")

    records = getattr(nusc, table_name)
    if not records:
        raise ValueError(f"NuScenes table '{table_name}' is empty.")

    # Check if records have timestamps (important for safety)
    if not records or 'timestamp' not in records[0]:
        raise ValueError(f"Records in table '{table_name}' do not seem to have a 'timestamp' key.")

    # Extract timestamps (list comprehension is efficient)
    record_timestamps = [rec['timestamp'] for rec in records]

    # --- Optional Check: Verify sorting (can be slow, disable for performance) ---
    # is_sorted = all(record_timestamps[i] <= record_timestamps[i+1] for i in range(len(record_timestamps)-1))
    # assert is_sorted, f"Table '{table_name}' is not sorted by timestamp, binary search invalid."
    # --- End Optional Check ---


    # Use bisect_left to find the insertion point (index)
    # This gives the index where the timestamp would be inserted to maintain order
    pos = bisect.bisect_left(record_timestamps, timestamp)

    # Handle edge cases: timestamp is before the first record or after the last
    if pos == 0:
        return records[0]
    if pos == len(record_timestamps):
        return records[-1]

    # Compare the timestamp with the records before and at the insertion point
    before_timestamp = record_timestamps[pos - 1]
    after_timestamp = record_timestamps[pos]

    if abs(after_timestamp - timestamp) < abs(timestamp - before_timestamp):
        # The timestamp at 'pos' is closer
        return records[pos]
    else:
        # The timestamp at 'pos - 1' is closer (or equally close, choosing earlier)
...
def get_pointwise_fused_pointcloud(nusc: NuScenes, sample: Dict[str, Any], allowed_sensors: List[str]) -> LidarPointCloud:
    ...
    fused_point_cloud = LidarPointCloud(points)
    fused_point_cloud.timestamps = timestamps  # add timestamps manually
    ...
    for sensor in sample['data'].keys():
        ...
        # ego_pose_t_min = nusc.getclosest('ego_pose', t_min)
        ego_pose_t_min = get_closest_record_optimized(nusc, 'ego_pose', t_min)
        # ego_pose_t_max = nusc.getclosest('ego_pose', t_max)
        ego_pose_t_max = get_closest_record_optimized(nusc, 'ego_pose', t_max)
...
def get_rigid_fused_pointcloud(nusc: NuScenes, sample: Dict[str, Any], allowed_sensors: List[str]) -> LidarPointCloud:
    """ Returns a fused lidar point cloud for the given sample.
    ...
    for sensor in sample['data'].keys():
        ...
        sensor_ego_pose = get_closest_record_optimized(nusc, 'ego_pose', sd['timestamp'])
...
def get_boxes(nusc: NuScenes, sample: Dict[str, Any]) -> List[Box]:
    ...
    boxes = list(map(nusc.get_box, sample['anns']))
...
def main(nusc, val_list, indice, nuscenesyaml, args, config):
    ...
    while True:
        ...
        if load_mode == 'pointwise':
            sensor_fused_pc = get_pointwise_fused_pointcloud(nusc, my_sample, allowed_sensors=sensors)
        elif load_mode == 'rigid':
            sensor_fused_pc = get_rigid_fused_pointcloud(nusc, my_sample, allowed_sensors=sensors)
        else:
        ...
        ########### get boxes #####################
        boxes = get_boxes(nusc, my_sample)
        ...
        object_tokens = [nuscenes.get('sample_annotation', box_token)['instance_token'] for box_token in
                         boxes_token]  # Uses sample_annotation data to get instance_token fore each bb
        # Extract object categories
        object_category = [nuscenes.get('sample_annotation', box_token)['category_name'] for box_token in
        ...
        for category in object_category:
            ...
            for label_key, label_name_in_yaml in nuscenesyaml['labels'].items():
            ...
            if not found_match:
                print(
                    f"Warning: Category '{category}' not found in nuscenes.yaml mapping. Using 'Unknown' label.")
                # --- CHANGE: Use UNKNOWN_LEARNING_INDEX ---
                converted_object_category.append(UNKNOWN_LEARNING_INDEX)
        ...
        # ego_pose = nusc.getclosest('ego_pose', my_sample['timestamp'])
        ego_pose = get_closest_record_optimized(nusc, 'ego_pose', my_sample['timestamp'])
        ...
        ref_sd = nusc.get('sample_data', my_sample['data']['LIDAR_TOP'])
    ...
    while int(i) < 10000:  # Assuming the sequence does not have more than 10000 frames to avoid infinite loops
        ...
        sample = nuscenes.get('sample', frame_dict['sample_token'])
        # ego_pose = nuscenes.getclosest('ego_pose', sample['timestamp'])
        ego_pose = get_closest_record_optimized(nusc, 'ego_pose', sample['timestamp'])
        ...
        sample = nuscenes.get('sample', frame_dict['sample_token'])
        boxes = get_boxes(nuscenes, sample)
...
if __name__ == '__main__':
    ...
    parse.add_argument('--config_path', type=str, default='config_nuscenes.yaml') # Configuration file path with default: "config.yaml"
    ...
    if args.dataset=='nuscenes': # check dataset type
        ...
        nuscenes = NuScenes(version='v1.0-trainval',
    ...
    for i in range(args.start,args.end):
        ...
        main(nuscenes, val_list, indice=i,
