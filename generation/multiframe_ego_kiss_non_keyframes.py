...
def main(trucksc, val_list, indice, truckscenesyaml, args, config):
    ...
    for i, group in enumerate(groups):
        ...
        if load_mode == 'pointwise':
            sensor_fused_pc = get_pointwise_fused_pointcloud(trucksc=trucksc, sample=sample, allowed_sensors=sensors)
        elif load_mode == 'rigid':
            sensor_fused_pc = get_rigid_fused_pointcloud(trucksc=trucksc, sample=sample, allowed_sensors=sensors)
        else:
        ...
        # visualize_pointcloud_bbox(points=sensor_fused_pc.points.T, boxes=boxes_ego, title=f"Fused sensor PC in ego coordinates - Frame {i}")
        ...
        ref_sd = trucksc.get('sample_data', sample['data'][sensors[0]])
    ...
    while int(i) < 10000:  # Assuming the sequence does not have more than 10000 frames to avoid infinite loops
        ...
        if not is_key_frame:  # only use key frame as GT
            i = i + 1
            print(f"   Skipping frame {i} because it is not a key frame.")
        ...
        try:  # avoid concatenate an empty array
            ...
            print(f"Concatenated {temp.shape[0]} objects points and {point_cloud.T.shape[0]} static points")
        except:
        ...
        try:
            ...
            print(f"Concatenated {temp.shape[0]} objects points and {point_cloud_with_semantic.T.shape[0]} static points")
        except:
